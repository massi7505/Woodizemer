  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-0.5">{total} product views recorded</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              ...
              <th className="text-left px-4 py-3 font-medium text-slate-600">Viewed At</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {loading && <tr><td colSpan={3} className="py-16 text-center text-slate-400">Loading…</td></tr>}
            {!loading && history.length === 0 && <tr><td colSpan={3} className="py-16 text-center text-slate-400">No history yet</td></tr>}
            {history.map(h => (

...
export default function ProductDetailPage() {
  ...
  if (error || !product) return (
    <div className="text-center py-24 text-slate-400">
      ...
      <p className="font-medium text-slate-600">Product not found</p>
      <button onClick={() => router.back()} className="mt-4 btn-secondary text-sm">Go back</button>
    </div>
  )
    ...
    <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {product.toxic && <span className="badge badge-amber">⚠ Toxic</span>}
              {product.cmr && <span className="badge badge-red">⚠ CMR</span>}
              {!product.active && <span className="badge badge-slate">Inactive</span>}
            </div>
            {product.cas && (
      ...
      {(product.toxic || product.cmr) && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-4 flex items-start gap-3">
          ...
          <div className="text-sm text-amber-800">
            <strong>Safety Warning:</strong> This product is classified as{' '}
            {[product.toxic && 'toxic', product.cmr && 'CMR (carcinogenic, mutagenic, or reprotoxic)'].filter(Boolean).join(' and ')}.
      ...
      <div className="card p-6 mb-4">
        ...
        <div className="space-y-3">
          {[
            { icon: Building2, label: 'Building', value: location.building.name, color: 'sky' },
            { icon: DoorOpen, label: 'Room', value: location.name, color: 'violet' },
            { icon: Archive, label: 'Cabinet', value: product.shelf.cabinet.name, color: 'emerald' },
            { icon: Layers, label: 'Shelf', value: product.shelf.name, color: 'amber' },
          ].map((step, i) => (
      ...
      <div className="card p-6">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
          {[
            ...
            { label: 'Physical State', value: product.physicalState },
            { label: 'Molar Mass', value: product.molarMass ? `${product.molarMass} g/mol` : null },
            { label: 'Purity', value: product.purityPercent ? `${(product.purityPercent * 100).toFixed(1)}%` : null },
            { label: 'Packaging', value: product.packagingValue ? `${product.packagingValue} ${product.packagingUnit}` : null },
            { label: 'Stock Type', value: product.globalStockType },
        ...
        {product.comment && (
          <div className="mt-4 pt-4 border-t border-slate-100">
            <div className="text-xs text-slate-400 font-medium mb-1">Comment</div>
